...
public class ObjectMapperBsonParser extends ParserBase implements Wrapper<BsonReader> {

    private final AbstractBsonReader reader;

    private ObjectMapperBsonParser.ParseContext context;
    /**
     * The ObjectCodec used to parse the Bson object(s)
     */
    private ObjectCodec _codec;

    ObjectMapperBsonParser(IOContext ctxt, int jsonFeatures, AbstractBsonReader reader) {
        super(ctxt, jsonFeatures);
        this.reader = Objects.requireNonNull(reader, "reader");
        this.context = new ObjectMapperBsonParser.ParseContext();
    }
    ...
    /**
     * Read (parse) current bson value and stored it in local {@link ObjectMapperBsonParser.ParseContext} cache.
     */
